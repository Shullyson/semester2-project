...
const Professors = ({ handleClose }) => {
  ...
  const fetchProfessors = useCallback(() => {
    axios.get('http://localhost:5000/adminprofessors')
      .then(response => {
  ...
  const addProfessor = () => {
    axios.post('http://localhost:5000/adminprofessors', newProfessor)
      .then(() => {
        fetchProfessors();
  ...
  const updateProfessor = () => {
    axios.put(`http://localhost:5000/adminprofessors/${currentProfessor.id}`, currentProfessor)
      .then(() => {
        fetchProfessors();
        setEditing(false);
  ...
  const deleteProfessor = (id) => {
    axios.delete(`http://localhost:5000/adminprofessors/${id}`)
      .then(() => {
        fetchProfessors();
      })
      .catch(error => {
